...
const teams: Map<number, StringArray> = new Map();

function initTeam(teamId: number) {
  if (!teams.has(teamId)) {
    teams.set(teamId, {
      goalsFor: 0,
      goalsForHome: 0,
...
function addTeamScore(teamId: number, goalsFor: number, goalsAgainst: number, isHomeGame: boolean) {
  initTeam(teamId);
...
function getTeamStats(matches: Array<Match>): Map<number, StringArray> {
